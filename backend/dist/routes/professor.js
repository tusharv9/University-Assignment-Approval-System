...
function signatureHash(signature) {
    return crypto_1.default.createHash('sha256').update(signature).digest('hex');
}
// GET /professor/dashboard - Pending assignments for the professor/HOD
router.get('/dashboard', auth_1.authenticateToken, (0, auth_1.requireRole)('PROFESSOR', 'HOD'), async (req, res) => {
    try {
...
router.get('/assignments/:id/review', auth_1.authenticateToken, (0, auth_1.requireRole)('PROFESSOR', 'HOD'), async (req, res) => {
    try {
        const professorId = req.user.id;
        const idParam = req.params.id;
...
router.post('/assignments/:id/approve/request-otp', auth_1.authenticateToken, (0, auth_1.requireRole)('PROFESSOR'), async (req, res) => {
    try {
        const professorId = req.user.id;
        ...
        const sent = await (0, email_1.sendOtpEmail)(professorEmail, otp);
        if (!sent) {
            otpStore.delete(key);
            return res.status(500).json({
                success: false,
...
router.post('/assignments/:id/approve/verify', auth_1.authenticateToken, (0, auth_1.requireRole)('PROFESSOR'), async (req, res) => {
    try {
        const professorId = req.user.id;
        ...
        const { otp, remarks, signature } = req.body;
        ...
        await prisma_1.default.$transaction([
            prisma_1.default.assignment.update({
                where: { id: assignmentId },
                data: { status: 'APPROVED' }
            }),
            prisma_1.default.assignmentHistory.create({
                data: {
                    assignmentId,
                    reviewerId: professorId,
                    action: 'APPROVED',
...
router.get('/forward-recipients', auth_1.authenticateToken, (0, auth_1.requireRole)('PROFESSOR', 'HOD'), async (req, res) => {
    try {
...
router.post('/assignments/:id/forward', auth_1.authenticateToken, (0, auth_1.requireRole)('PROFESSOR', 'HOD'), async (req, res) => {
    try {
        const professorId = req.user.id;
        const idParam = req.params.id;
        const assignmentId = typeof idParam === 'string' ? parseInt(idParam, 10) : NaN;
        ...
        const newReviewer = await prisma_1.default.user.findFirst({
            where: {
                id: newReviewerId,
...
router.post('/assignments/:id/reject', auth_1.authenticateToken, (0, auth_1.requireRole)('PROFESSOR'), async (req, res) => {
    try {
        const professorId = req.user.id;
        const idParam = req.params.id;
        const assignmentId = typeof idParam === 'string' ? parseInt(idParam, 10) : NaN;
        ...
        await (0, email_1.sendRejectionEmail)(assignment.student.email, assignment.title, trimmedRemark);
        return res.json({
            success: true,
